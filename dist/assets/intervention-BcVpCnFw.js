import"./modulepreload-polyfill-CSRv37U6.js";var e=[`Notice the urge without acting on it. What is the anxiety actually telling you?`,`Your discomfort is temporary. What would happen if you sat with this feeling for 5 minutes?`,`OCD demands certainty. Can you tolerate the uncertainty instead of seeking reassurance?`,`Rate your anxiety 1–10. Research shows it peaks and then falls — can you wait for the drop?`,`What value are you protecting by not giving in? What matters more than the anxiety right now?`,`Avoidance teaches your brain the site is dangerous. What would you tell a friend in your position?`,`Every time you resist a compulsion, you weaken the OCD pathway. This moment counts.`];function t(){return e[Math.floor(Math.random()*e.length)]}function n(e){return String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function r(){let e=new URLSearchParams(location.search);return{site:e.get(`site`)||`this site`,mechanism:e.get(`mechanism`)||null,term:e.get(`term`)||null,label:e.get(`label`)||null,matchedTheme:e.get(`matchedTheme`)||null,query:e.get(`query`)||null,score:parseFloat(e.get(`score`)||`0`)||null}}function i(e){return e.trim().replace(/^https?:\/\//,``).replace(/^www\./,``).replace(/\/.*$/,``).toLowerCase()}function a({site:e,mechanism:t,term:r,label:i,matchedTheme:a,query:o,score:s}){let c=s?` (${Math.round(s*100)}% match)`:``,l=o?`<em>"${n(o)}"</em>`:`your search`;return t===`keyword`?`
